...
public class ReplayStartCommand extends SubCommand {
    ...
    public boolean execute(CommandSender cs, Command cmd, String label, String[] args) {
        ...
        if (duration <= 0) {
            ...
            if (Platform.isFolia()) {
                if (cs instanceof Player psender)
                    psender.getScheduler().runDelayed(ReplaySystem.getInstance(), (e) -> task.run(), null, duration * 20);
                else
                    Bukkit.getGlobalRegionScheduler().runDelayed(ReplaySystem.getInstance(), (e) -> task.run(), duration * 20);
