...
public class ReplayListener extends AbstractListener {
    ...
    public void onDamage(EntityDamageEvent e) {
        if (e.getEntity() instanceof Player p) {
            if (ReplayHelper.replaySessions.containsKey(p.getName())) e.setCancelled(true);
        }
    }
    ...
    public void onPickup(PlayerPickupItemEvent e) {
        ...
        if (isReplayItem) e.setCancelled(true);
    }
        ...
        if (ReplayHelper.replaySessions.containsKey(p.getName())) {
            ...
            BukkitRunnable task = new BukkitRunnable() {
                ...
                public void run() {
                    for (INPC npc : replayer.getNPCList().values()) {
                        npc.despawn();
                        if (ReplayHelper.isInRange(p.getLocation(), npc.getLocation())) npc.respawn(p);
                    }
                }
            };
