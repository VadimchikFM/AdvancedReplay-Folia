...
public class Recorder {
    ...
    public void start() {
        ...
        if (Platform.isFolia())
            task = Bukkit.getAsyncScheduler().runAtFixedRate(ReplaySystem.getInstance(), (e) -> this.run.run(), 50, 50, TimeUnit.MILLISECONDS);
    ...
    public void createSpawnAction(Player player, Location loc, boolean first) {
        ...
        if (!Bukkit.getOnlineMode() && ConfigManager.USE_OFFLINE_SKINS) {
            ...
            if (Platform.isFolia()) Bukkit.getAsyncScheduler().runNow(ReplaySystem.getInstance(), (e) -> task2.run());
            else task2.runTaskAsynchronously(ReplaySystem.getInstance());
        }
